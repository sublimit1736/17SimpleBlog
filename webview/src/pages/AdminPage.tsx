      ...
      <div className="container">
        ...
        {tab === 'pending' && (
          loading ? <Loading center /> : pending.length === 0 ? (
            <div className="empty-state"><div className="empty-icon">✅</div><p>暂无待审核文章</p></div>
          ) : (
              ...
              {pending.map((a) => (
                <div key={a.id} className={styles.row}>
                  <div className={styles.rowInfo}>
                    ...
                    <span className={styles.meta}>by {a.authorName} · {new Date(a.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className={styles.rowActions}>
                    <Button size="sm" onClick={() => approveArticle(a.id)}>通过</Button>
                    <Button size="sm" variant="danger" onClick={() => rejectArticle(a.id)}>拒绝</Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab === 'comments' && (
          loading ? <Loading center /> : comments.length === 0 ? (
            <div className="empty-state"><div className="empty-icon">💬</div><p>暂无评论</p></div>
          ) : (
            <div className={styles.list}>
              {comments.map((c) => (
                <div key={c.id} className={styles.row}>
                  <div className={styles.rowInfo}>
                    ...
                    <span className={styles.meta}>by {c.authorName} · <Link to={`/article/${c.articleId}`}>查看文章</Link></span>
                  </div>
              ...
              <div className={styles.list}>
                {users.map((u) => (
                  <div key={u.id} className={styles.row}>
                    <div className={styles.rowInfo}>
                      <Link to={`/profile/${u.id}`} className={styles.articleTitle}>{u.username}</Link>
                      <span className={styles.meta}>UID: {u.id} · {u.role}</span>
                    </div>
                    <div className={styles.rowActions}>
                      {u.role === 'ADMIN'
                        ? <Button size="sm" variant="secondary" onClick={() => demoteUser(u.id)}>撤销管理员</Button>
