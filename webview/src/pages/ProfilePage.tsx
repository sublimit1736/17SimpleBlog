...
const ProfilePage: React.FC = () => {
  ...
  const handleUpdateUsername = async () => {
    if (!newUsername.trim()) { showToast('请输入新用户名', 'warning'); return; }
    setSaving(true);
    try {
    ...
    } catch { showToast('修改失败', 'error'); } finally { setSaving(false); }
  };

  const handleChangePassword = async () => {
    if (!oldPassword || !newPassword) { showToast('请填写密码', 'warning'); return; }
    if (newPassword.length < 6) { showToast('新密码至少6位', 'warning'); return; }
    setSaving(true);
    try {
  ...
  if (!profile) {
    return <Layout><div className="container"><p>用户不存在</p></div></Layout>;
  }
            ...
            {isSelf && (
              <div className={styles.editBtns}>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => { setNewUsername(profile.username); setEditModal(true); }}
                >
                  ✏ 修改用户名
                </Button>
                <Button
